...
// ─── Custom scene (generated by AI from image) ────────────────────────────────

interface CustomSceneObject {
  id: string;
  type: "box" | "cylinder" | "sphere" | "ground";
  width?: number; height?: number; depth?: number; diameter?: number;
...
function buildCustomScene(scene: Scene, config: CustomSceneConfig, addFn: AddFn) {
  const [br, bg, bb] = config.backgroundColor ?? [0.08, 0.12, 0.18];
  scene.clearColor = new Color4(br, bg, bb, 1);

  const sun = new DirectionalLight("sun", new Vector3(-0.6, -1.5, 0.8), scene);
  sun.intensity = 2.2;
  ...
  for (const obj of config.objects ?? []) {
    ...
    if (obj.type === "box") {
      mesh = MeshBuilder.CreateBox(obj.id, {
        width: obj.width ?? 1, height: obj.height ?? 1, depth: obj.depth ?? 1
      }, scene);
    } else if (obj.type === "cylinder") {
      mesh = MeshBuilder.CreateCylinder(obj.id, {
        diameter: obj.diameter ?? 1, height: obj.height ?? 1, tessellation: 16
      }, scene);
    } else if (obj.type === "sphere") {
      mesh = MeshBuilder.CreateSphere(obj.id, {
        diameter: obj.diameter ?? 1, segments: 12
      }, scene);
    } else if (obj.type === "ground") {
      mesh = MeshBuilder.CreateGround(obj.id, {
        width: obj.width ?? 20, height: obj.depth ?? 20
      }, scene);
    }
    ...
    const [cr, cg, cb] = obj.color ?? [0.6, 0.6, 0.6];
    mesh.material = pbr(
      `${obj.id}_m`, scene,
      new Color3(cr, cg, cb),
      obj.metallic ?? 0.1,
      obj.roughness ?? 0.7,
      obj.emissive ? new Color3(obj.emissive[0], obj.emissive[1], obj.emissive[2]) : undefined
    );
  ...
  addFn({ type: "simulation.started", detail: `Escena personalizada con ${config.objects?.length ?? 0} objetos cargada.` });
}
...
export function SimulationCanvas({ scenario }: Props) {
  ...
  useEffect(() => {
    ...
    // Escena custom generada por IA
    const rawConfig = scenario?.babylonConfig as (CustomSceneConfig & { sceneType?: string }) | undefined;
    const isCustom = rawConfig?.sceneType === "custom" || (Array.isArray(rawConfig?.objects) && (rawConfig?.objects?.length ?? 0) > 0);
    const customConfig: CustomSceneConfig | undefined = isCustom ? {
      camera: rawConfig?.camera,
      backgroundColor: rawConfig?.backgroundColor,
      objects: (rawConfig?.objects && rawConfig.objects.length > 0) ? rawConfig.objects : [
        { id: "floor", type: "ground", width: 20, depth: 20, color: [0.35, 0.35, 0.35], metallic: 0, roughness: 0.9 },
        { id: "struct1", type: "box", width: 4, height: 3, depth: 4, position: [-3, 1.5, 0], color: [0.75, 0.65, 0.55], metallic: 0, roughness: 0.8 },
        { id: "struct2", type: "box", width: 3, height: 2, depth: 3, position: [3, 1, 0], color: [0.6, 0.7, 0.8], metallic: 0.2, roughness: 0.6 },
        { id: "accent", type: "sphere", diameter: 1.2, position: [0, 3.5, 0], color: [0.9, 0.8, 0.2], metallic: 0.8, roughness: 0.2, emissive: [0.4, 0.35, 0.05] },
      ]
    } : undefined;
    if (customConfig) {
      const alpha  = customConfig.camera?.alpha ?? 1.2;
      const beta   = customConfig.camera?.beta  ?? 0.8;
      const radius = customConfig.camera?.radius ?? 15;
      const cam = new ArcRotateCamera("cam", alpha, beta, radius, Vector3.Zero(), scene);
      cam.attachControl(canvas, true);
      cam.lowerRadiusLimit = 2;
      cam.upperRadiusLimit = radius * 3;
      buildCustomScene(scene, customConfig, addEvent);
    } else {
